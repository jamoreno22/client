...
func main() {
	...
	for {
		...
		if scanner.Scan() {
			...
			fmt.Println(localPageInfo)

			fmt.Println(mReads(comm.Domain, localPageInfo.DnsIP, &comm, localPageInfo.Ip))
		}
	}
...
func mReads(zfName string, ip string, comm *l3.Command, pageIP string) string {
	if len(consList) != 0 {
